...
// got overflow with u32 needed to use u64!!
    assert_eq!(toboggan_path(vector, pattern), 7);
     let vector2: Vec<Vector> = vec![Vector{right: 1, down: 1}, Vector{right: 3, down: 1}, Vector{right: 5, down: 1}, Vector{right: 7, down: 1}, Vector{right: 1, down: 2}];
    let res: Vec<u64> = vector2.iter().map(|x| toboggan_path(*x, pattern2)).collect::<Vec<u64>>();

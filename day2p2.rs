...
fn main() {
    fn test(text: &str) -> bool {
        ...
        Some(x) => {
            let caps = x;
            let test_value =  Parsed {min: caps.get(1).map_or("", |m| m.as_str()).parse::<u32>().unwrap() -1, 
                max: caps.get(2).map_or("", |m| m.as_str()).parse::<u32>().unwrap() -1,
            ...
            while j < test_value.password.len() {
                if pass_vector[j] == test_value.letter && (j == test_value.min as usize || j == test_value.max as usize) {
                    matches = matches + 1;
                }
                j = j + 1; 
            }
...
"2-9 c: ccccccccc"];
 assert_eq!(test(test_vec[0]), true);
  assert_eq!(test(test_vec[1]), false);
    assert_eq!(test(test_vec[2]), false);
